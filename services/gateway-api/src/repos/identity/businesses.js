...
router.post("/", requireActor, async (req, res, next) => {
  try {
    const legal_name = (req.body && req.body.legal_name || "").trim();
...
router.get("/:id", requireActor, requireBusinessRole({ anyOf: ["OWNER_PRIMARY", "OWNER", "ADMIN", "EMPLOYEE", "FINANCE_APPROVER"] }), async (req, res, next) => {
  try {
...
router.post("/:id/kyb/start", requireActor, requireBusinessRole({ anyOf: ["OWNER_PRIMARY", "ADMIN"] }), async (req, res, next) => {
  try {
...
router.post("/:id/kyb/submit", requireActor, requireBusinessRole({ anyOf: ["OWNER_PRIMARY", "ADMIN"] }), async (req, res, next) => {
  try {
    const business_id = req.params.id;
    if (!isUuid(business_id)) return res.status(400).json({ ok: false, error: "invalid business id" });
...
router.post("/:id/members/invite", requireActor, requireBusinessRole({ anyOf: ["OWNER_PRIMARY", "ADMIN"] }), async (req, res, next) => {
  try {
    const business_id = req.params.id;
    if (!isUuid(business_id)) return res.status(400).json({ ok: false, error: "invalid business id" });
...
router.post("/:id/roles/assign", requireActor, requireBusinessRole({ anyOf: ["OWNER_PRIMARY"] }), async (req, res, next) => {
  try {
    const business_id = req.params.id;
    if (!isUuid(business_id)) return res.status(400).json({ ok: false, error: "invalid business id" });
