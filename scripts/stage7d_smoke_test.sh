...
request_json "POST" "$BASE_URL/public/v1/finance/approvals/$PAYMENT_INTENT_ID/vote" \
  "{\"vote\":\"approve\"}" \
...
request_json "POST" "$BASE_URL/public/v1/identity/spaces/switch" \
  "{\"space_id\":\"$SPACE_ID\"}" \
  -H "Content-Type: application/json" \
  -H "x-session-id: $MOBILE_SESSION_ID" \
  -H "x-web-session-id: $WEB_SESSION_ID"
